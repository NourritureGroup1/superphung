...
public class ShareMomentTask extends AsyncTask<String, Void, String> {
	...
	public String addMoment()
	{
		if (helpers.haveNetworkConnection(context))
		{
			...
			String response = fileUpload.executeMultiPartRequest(Globals.MainActivityDatas.urls.get("POST").get("share_moment"), file, file.getName(), "File Upload test Hydrangeas.jpg description") ;
			System.out.println("Response : "+response);
			
			/*List<NameValuePair> parameters = new ArrayList<NameValuePair>(2);
			parameters.add(new BasicNameValuePair("creator", Globals.MainActivityDatas.user.getId()));
			parameters.add(new BasicNameValuePair("description", moment.getDescription()));
			parameters.add(new BasicNameValuePair("image", ""));*/
			//post(Globals.MainActivityDatas.urls.get("POST").get("share_moment"),parameters);
	...
	public void post(String url, List<NameValuePair> nameValuePairs) {
		System.out.println("JE POST "+moment.getImgUrl());
		HttpClient client = new DefaultHttpClient();
		client = sslClient(client);
		
            /*entityBuilder.addTextBody("creator", Globals.MainActivityDatas.user.getId());
            entityBuilder.addTextBody("description", moment.getDescription());
            entityBuilder.addBinaryBody("image", (new File(moment.getImgUrl())));*/
            
            
            JsonObject json = new JsonObject();
            json.addProperty("creator", Globals.MainActivityDatas.user.getId());
            json.addProperty("description", moment.getDescription());
            System.out.println("url img : "+moment.getImgUrl());
            Ion.with(context)
            .load(url)
