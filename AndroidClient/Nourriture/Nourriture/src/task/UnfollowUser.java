...
public class UnfollowUser extends AsyncTask<String, Void, String> {
	...
	public UnfollowUser(Context c_, User user_) {
		context = c_;
		other = user_;
	}
	...
	private void unfollow() {
		...
		if (!following.equals("")) {
			if (following.contains(other.getId())) {
				if (following.equals(other.getId())) {
					Globals.MainActivityDatas.user.setFollowing("");
				}
				else {
					String tmp = Globals.MainActivityDatas.user.getFollowings();
					tmp = tmp.replace(","+other.getId(), "");
					tmp = tmp.replace(other.getId()+",", "");
