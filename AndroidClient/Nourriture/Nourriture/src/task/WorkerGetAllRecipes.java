...
import com.superphung.nourriture.SliderElementsRecipe;
import com.superphung.nourriture.helpers;

public class WorkerGetAllRecipes extends AsyncTask<String, Void, String> {
	...
	private ArrayList<Recipe> listAllIngredients;

	public WorkerGetAllRecipes(Context c_, View rootview_, BrowseRecipeAdapter adapteur_, ImageLoader imageLoader_) {
		context = c_;
		rootview = rootview_;
		listAllIngredients = new ArrayList<Recipe>();
	...
	protected void onPostExecute(String result) {
		...
		Globals.listRecipe = listAllIngredients;
		...
		if (gallery != null) {
			...
			gallery.setOnItemClickListener(new OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> arg0, View v, int position, long arg3) {
					Intent myIntent = new Intent(context, SliderElementsRecipe.class);
					Bundle b = new Bundle();
	...
	public String  getAll()
	{
		if (helpers.haveNetworkConnection(context))
		{
			...
			String response = helpers.getDatas(Globals.MainActivityDatas.urls.get("GET").get("recipe"),parameters, "GET");
			if (!helpers.isNumeric(response)) {
				try {
					...
					for(int i=0;i<allIngredientsjson.length();i++)
					{
						...
						Recipe tmp_all = new Recipe(curr_all.getString("name"),curr_all.getString("_id"),curr_all.getString("creator"),curr_all.getString("description"),URL_API+curr_all.getString("imgUrl"),null,curr_all.getString("ingredients"),curr_all.getString("steps"));							
