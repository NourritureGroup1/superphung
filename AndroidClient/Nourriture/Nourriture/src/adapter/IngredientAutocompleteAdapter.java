...
public class IngredientAutocompleteAdapter extends ArrayAdapter<Ingredient> implements Filterable {
	...
	public IngredientAutocompleteAdapter(Context context_, int resource,
			...
			protected FilterResults performFiltering(CharSequence constraint) {
				...
				if(constraint != null) {
					...
					if (CustomListViewValuesArr != null) {
						ArrayList<String> list_autocomplete = new ArrayList<String>();
						for (int i = 0; i < CustomListViewValuesArr.size(); ++i) {
							if (CustomListViewValuesArr.get(i).getName().contains(constraint))
								list_autocomplete.add(CustomListViewValuesArr.get(i).getName());
						}
						filterResults.values = list_autocomplete;
						filterResults.count = list_autocomplete.size();
					}
				}
